...
We additionally benchmark GMM-BIC as an 11th comparison index. GMM-BIC achieves $30.7\% \pm 0.8\%$ accuracy (ranked 8th of 11, comparable to Davies--Bouldin at $30.0\%$). CBV significantly outperforms GMM-BIC ($p < 0.001$, paired Wilcoxon), confirming that CBV's modality-testing approach captures structure that the standard model-based BIC selection misses. The low accuracy of GMM-BIC---despite its widespread use for mixture order selection---underscores the difficulty of the cluster-count estimation problem and motivates the need for dedicated cluster validation indices.
...
\caption{Accuracy comparison across 10 indices on 58 datasets. CBV achieves the lowest reported variance (seed-to-seed only; CBV is deterministic across $k$-means seeds).}
\label{fig:accuracy}
\end{figure}
...
On a comprehensive benchmark of 58 datasets evaluated against 10 established CVIs across 5 random seeds, CBV achieves $51.4\% \pm 0.8\%$ exact-match accuracy, statistically competitive with the top-performing indices. CBV exhibits the lowest seed-to-seed variance (an artifact of its determinism across $k$-means seeds; see \S\ref{sec:per-seed}). Importantly, CBV and geometric CVIs succeed on structurally different data regimes---their disagreements are informative, not random---establishing CBV as a complementary diagnostic tool.
